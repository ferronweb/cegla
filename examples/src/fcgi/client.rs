use bytes::Bytes;
use cegla_fcgi::client::{handshake, CgiBuilder};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let (send_req, conn) = handshake(socket, false).await?;

  tokio::spawn(conn);

  let (response, _stderr) = send_req.send_request(request, CgiBuilder::new()).await?;
